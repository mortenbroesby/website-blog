...
This week I was faced with a challenge. How do we predict how many users will access our website?
...
This, in turn, limits how many users can use the services on consuming websites with pages taht use such a SAAS products. Take for example payments.

At any given second, no more than 100 users will be able to pay. This is a rate limit.

The 101'st user who shows up will be blocked. This is objectively a terrible user experience.
...
If we implement the Token Bucket strategy to rate limit an API, then it allows us to set a request rate
- i.e. the rate at which tokens are added to the bucket - with the ability to burst above this rate for
a short period, until we have drained the capacity of the bucket.
...
of a previously mentioned SAAS product. The user are able to use our website just like they were before, and once our bucket is empty, we just fall back to the normal API call.
